...
fn bench_get_chunk_by_ordinal(c: &mut Criterion) {
    ...
    c.bench_function("get_chunk_by_ordinal", |b| {
        b.iter(|| {
            let rt = tokio::runtime::Runtime::new().unwrap();
            rt.block_on(async {
                index.get_chunk_by_ordinal(black_box("test.md"), black_box(1)).await
            })
        })
    });
}
...
fn bench_file_reindexing(c: &mut Criterion) {
    ...
    c.bench_function("file_reindexing", |b| {
        b.iter(|| {
            let rt = tokio::runtime::Runtime::new().unwrap();
            rt.block_on(async {
                let mut index = knowledge_loom::bm25::BM25Index::new(kb_root.to_str().unwrap()).await;
                index.index_file(black_box(&test_file), black_box(content)).await
            })
        })
    });
}
...
fn bench_corpus_reingestion(c: &mut Criterion) {
    ...
    let vault_state = std::thread::spawn(move || {
        let rt = tokio::runtime::Runtime::new().unwrap();
        rt.block_on(async {
            knowledge_loom::vault::VaultState::new(&kb_root_clone).await
        })
    }).join().unwrap();
