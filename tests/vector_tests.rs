...
mod tests {
    ...
    async fn test_vector_index_upsert() {
        ...
        match index
            .upsert_embedding(&test_path, Some("Test Heading"), "Test content", &embedding)
            .await
        {
            Ok(_) => {
    ...
    async fn test_vector_index_remove() {
        ...
        // Remove embedding
        index
            .remove_embedding(&test_path, Some("Test Heading"))
            .await
            .unwrap();
    ...
    async fn test_vector_index_unique_constraint() {
        ...
        // Add first embedding
        index
            .upsert_embedding(&test_path, Some("Test Heading"), "Content 1", &embedding1)
            .await
            .unwrap();

        // Update with different embedding (should replace)
        index
            .upsert_embedding(&test_path, Some("Test Heading"), "Content 2", &embedding2)
            .await
            .unwrap();
