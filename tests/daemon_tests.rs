use knowledge_loom::daemon::{
    add_repo, expand_path, load_config, save_config, DaemonConfig, WatchRepo,
};
use std::fs;
...
fn test_config_dir_path() {
    ...
    assert!(
        dir.to_string_lossy().contains("knowledge-loom"),
        "config dir should contain knowledge-loom: {}",
        dir.display()
    );
}
...
fn test_daemon_add_repo() {
    ...
    knowledge_loom::daemon::add_repo("/tmp/mynotes", Some("notes"), &config_path).unwrap();
