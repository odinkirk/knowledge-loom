...
fn test_download_manager_creation() {
    ...
    let result = DownloadManager::new("https://example.com/model.onnx".to_string(), output_path);
...
fn test_download_manager_configuration() {
    ...
    let manager = DownloadManager::new("https://example.com/model.onnx".to_string(), output_path)
        .unwrap()
        .with_retries(5)
...
fn test_checksum_cross_module() {
    ...
    assert_eq!(
        checksum1, checksum2,
        "Checksum should be consistent across calls"
    );
}
