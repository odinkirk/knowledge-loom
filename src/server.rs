...
impl LoomServer {
    pub async fn new(kb_root: &str) -> Self {
        ...
        let maintenance = Arc::new(MaintenanceManager::new(
            kb_root.to_string(),
            vault.clone(), bm25.clone(), embed.clone(), vector.clone(), graph.clone(), brainjar.clone(),
        ));

        Self { kb_root: kb_root.to_string(), vault, bm25, embed, vector, graph, edits, maintenance, brainjar, search_engine }
    }

    pub fn tool_list(&self) -> Vec<ToolDef> {
        vec![
            ToolDef { name: "loom_search", description: "BM25 + semantic RRF search", schema: json!({"type":"object","properties":{"query":{"type":"string"},"top_k":{"type":"integer","default":10}},"required":["query"]}) },
            ToolDef { name: "loom_search_graph", description: "Graph entity/relationship traversal", schema: json!({"type":"object","properties":{"note":{"type":"string"}},"required":["note"]}) },
            ToolDef { name: "loom_search_smart", description: "LLM-decomposed search (requires BRAINJAR_PATH)", schema: json!({"type":"object","properties":{"query":{"type":"string"}},"required":["query"]}) },
            ToolDef { name: "loom_rank_notes", description: "PageRank influence ranking", schema: json!({"type":"object","properties":{}}) },
            ToolDef { name: "loom_find_connections", description: "Links and relationships for a note", schema: json!({"type":"object","properties":{"note":{"type":"string"}},"required":["note"]}) },
            ToolDef { name: "loom_find_path_between", description: "Shortest graph path between two notes", schema: json!({"type":"object","properties":{"note_a":{"type":"string"},"note_b":{"type":"string"}},"required":["note_a","note_b"]}) },
            ToolDef { name: "loom_detect_themes", description: "Louvain community/theme detection", schema: json!({"type":"object","properties":{}}) },
            ToolDef { name: "loom_list_files", description: "List all Markdown files", schema: json!({"type":"object","properties":{}}) },
            ToolDef { name: "loom_outline", description: "Heading hierarchy with line numbers", schema: json!({"type":"object","properties":{"file":{"type":"string"}},"required":["file"]}) },
            ToolDef { name: "loom_grep", description: "Regex search, optional file filter", schema: json!({"type":"object","properties":{"pattern":{"type":"string"},"file_filter":{"type":"string"}},"required":["pattern"]}) },
            ToolDef { name: "loom_read_section", description: "Read content under a heading", schema: json!({"type":"object","properties":{"file":{"type":"string"},"heading":{"type":"string"}},"required":["file","heading"]}) },
            ToolDef { name: "loom_read_lines", description: "Read exact line range", schema: json!({"type":"object","properties":{"file":{"type":"string"},"start":{"type":"integer"},"end":{"type":"integer"}},"required":["file","start","end"]}) },
            ToolDef { name: "loom_replace_lines", description: "In-place line range replacement", schema: json!({"type":"object","properties":{"file":{"type":"string"},"start":{"type":"integer"},"end":{"type":"integer"},"content":{"type":"string"}},"required":["file","start","end","content"]}) },
            ToolDef { name: "loom_insert_after_heading", description: "Insert content after a heading", schema: json!({"type":"object","properties":{"file":{"type":"string"},"heading":{"type":"string"},"content":{"type":"string"}},"required":["file","heading","content"]}) },
            ToolDef { name: "loom_append_to_file", description: "Append to file with blank-line separator", schema: json!({"type":"object","properties":{"file":{"type":"string"},"content":{"type":"string"}},"required":["file","content"]}) },
            ToolDef { name: "loom_create_note", description: "Create note from title and content", schema: json!({"type":"object","properties":{"title":{"type":"string"},"content":{"type":"string"}},"required":["title","content"]}) },
            ToolDef { name: "loom_edit_note", description: "Replace full note content", schema: json!({"type":"object","properties":{"file":{"type":"string"},"content":{"type":"string"}},"required":["file","content"]}) },
            ToolDef { name: "loom_apply_edit_preview", description: "Dry-run section replacement preview", schema: json!({"type":"object","properties":{"file":{"type":"string"},"heading":{"type":"string"},"proposed":{"type":"string"}},"required":["file","heading","proposed"]}) },
            ToolDef { name: "loom_link_notes", description: "Append wikilink from one note to another", schema: json!({"type":"object","properties":{"from":{"type":"string"},"to":{"type":"string"}},"required":["from","to"]}) },
            ToolDef { name: "loom_move_note", description: "Move note to new path", schema: json!({"type":"object","properties":{"from":{"type":"string"},"to":{"type":"string"}},"required":["from","to"]}) },
            ToolDef { name: "loom_delete_note", description: "Delete a note", schema: json!({"type":"object","properties":{"file":{"type":"string"}},"required":["file"]}) },
            ToolDef { name: "loom_reindex", description: "Rebuild all search indexes", schema: json!({"type":"object","properties":{}}) },
            ToolDef { name: "loom_index_status", description: "Health and chunk counts for all backends", schema: json!({"type":"object","properties":{}}) },
        ]
    }
        ...
        match name {
            ...
            "loom_search_smart" => {
                ...
                if !brainjar.is_available().await {
                    return Ok(serde_json::json!({"error": "BRAINJAR_PATH not configured"}).to_string());
                }
                // Use call_tool with a search command
                match brainjar.call_tool("search", serde_json::json!({"query": query})).await {
...
impl ServerHandler for LoomServer {
    fn get_info(&self) -> ServerInfo {
        ...
        info.instructions = Some("Knowledge Loom MCP server — 23 loom_* tools for search, navigation, editing, and graph analytics.".into());
        info
    }
